...
experiments_reduction_rows = csv_get_rows('experiments-reduction')
experiments_reduction_fields = row_to_dict(experiments_reduction_rows[0])
experiments_reduction_rows = experiments_reduction_rows[1:]

experiments_quality_rows = csv_get_rows('experiments-quality')
experiments_quality_fields = row_to_dict(experiments_quality_rows[0])
experiments_quality_rows = experiments_quality_rows[1:]

products_rows = csv_get_rows('products')
products_fields = row_to_dict(products_rows[0])
...
def generate_section_quality(product_type):
    text_intro = ''
    ...
    for i, row in enumerate(rows):
        if row[fields['product_type']].strip().lower() == product_type.lower().strip():
            
            _product = row[fields['product']]
            ...
            for product_row in products_rows:
                print(product_row[products_fields['product']], '->', _product)

                if product_row[products_fields['product']].strip().lower() == _product.lower().strip():
                    product_ad_2 = product_row[products_fields['product_ad_4']]
                    break
            ...
            # product = products_rows[products_fields['product']]
...
text = '## Quali applicazioni ha l\'ozono nell\'industria lattiero-casearia?'
with open('test.md', 'a', encoding=encoding) as f:
    f.write(f'{text}\n\n')
...
text = '## L\'ozono altera le proprietà sensoriali nei prodotti lattiero-caseari?'
